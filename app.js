...
// - Reserve => marked reserved (red) for the entire day
// - Available => green
// - Selected => blue
// - LocalStorage persistence
//
// RULES:
//  1. A seat/table reserved on a date is blocked for the WHOLE day (any time).
//     The same seat on a different date is free.
//  2. One reservation per customer (contact) per date.
//  3. Pax cannot exceed selected capacity.
...
// Returns the date string currently entered in the form (falls back to today).
...
// Returns Set of space IDs that are reserved on the given date (whole-day block).
function getReservedIdsForDate(date){
...
// Rule 1: A space is blocked for the entire day it's reserved.
function spaceConflictOnDate(date, chosenSpaceIds){
  const reservedIds = getReservedIdsForDate(date);
  return chosenSpaceIds.some(id => reservedIds.has(id));
}

// Rule 2: A contact may only have 1 reservation per date.
function customerAlreadyBookedOnDate(date, contact){
  const normalised = contact.trim().toLowerCase();
  return reservations.some(
    r => r.status !== "Cancelled" &&
         r.date === date &&
         r.contact.trim().toLowerCase() === normalised
  );
}

function updateVisualStates(){
  const viewDate = getFormDate();
  const reservedIds = getReservedIdsForDate(viewDate);
  ...
  // Mark selected (only if not reserved for the chosen date)
...
function renderReservationList(){
  ...
  resListEl.querySelectorAll("button[data-view]").forEach(btn => {
    btn.addEventListener("click", () => openReservationModal(btn.dataset.view));
  });
}
...
// Re-render floor whenever the date field changes so colours update live.
$("#date").addEventListener("change", () => {
  // Remove selected seats that are now reserved on the newly chosen date
  const reservedOnNewDate = getReservedIdsForDate(getFormDate());
  reservedOnNewDate.forEach(id => selectedIds.delete(id));
  updateVisualStates();
...
floor.querySelectorAll("[data-id]").forEach(node => {
  node.addEventListener("click", () => {
    ...
    // If reserved on the current view-date, show existing reservation details.
    if (node.classList.contains("reserved")){
      const viewDate = getFormDate();
      const r = reservations.find(
        x => x.status !== "Cancelled" && x.date === viewDate && x.spaceIds.includes(id)
      );
...
form.addEventListener("submit", (e) => {
  ...
  // Rule 1: Whole-day block — check if any chosen space is already taken on this date.
  if (spaceConflictOnDate(date, ids)){
    setHint("One or more selected spaces are already reserved on this date. Choose a different date or different seats.", "bad");
    return;
  }

  // Rule 2: One reservation per customer per date.
  if (customerAlreadyBookedOnDate(date, contact)){
    setHint("This contact already has a reservation on that date. Only 1 reservation per customer per day is allowed.", "bad");
    return;
  }

  // Rule 3: Pax vs capacity check.
